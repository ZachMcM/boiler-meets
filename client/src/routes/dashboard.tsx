...
function RouteComponent() {
  ...
  const { data: searchResults, isPending: searchPending } = useQuery({
    ...
    queryFn: async () =>
      searchQuery ? await searchUsers(searchQuery, searchPage) : null,
  ...
  const [notificationReload, setNotificationReload] = useState(
    [] as NotificationItem[]
  );
    ...
    ? (searchResults?.users || []).filter((user) => {
        ...
        if (matchFilter !== "all") {
          return callHistory?.some(
            (call) =>
              (call.calledUserId === user.id ||
                call.callerUserId === user.id) &&
  ...
  const destroyNotification = async (timestamp: number) => {
    ...
    const updatedList = currentNotifications.filter(
  ...
  const deleteMatch = async (match: Match) => {
    ...
    if (currentUserData?.data?.user.id) {
      ...
      try {
        //Self notifications update (Other user notifications are updated in users.ts /matches delete route)
        if (!currentUserData?.data?.user.notifications) return;
        const currentNotifications = JSON.parse(
          currentUserData.data.user.notifications
        ) as NotificationItem[];
        ...
        <div className="flex gap-2 mb-4">
          <Card className="mb-8 w-4/5">
            ...
            <CardContent>
              ...
              {/* Results List */}
              {(!globalSearch && matchesPending) ||
              (globalSearch && searchPending) ? (
                ...
                <div className="flex flex-col gap-2">
                  {filteredResults.map((item) => {
                    ...
                    const matchType = isMatch
                      ? (item as Match).matchType
                      : null;

                    const userMatchTypes =
                      globalSearch && user?.id
                        ? getUserMatchTypes(user.id)
                        : [];
                                  ...
                                  : callHistory
                                      ?.filter(
                                        (call) =>
                                          ...
                                          (call.calledUserId === user.id ||
                                            call.callerUserId === user.id)
                                      ) //If the filters change, update this and many other lines
        ...
        <Dialog open={unmatchDialog}>
          ...
          >
            <div className="flex flex-col space-y-4">
              <DialogTitle>
                Do you really want to unmatch with{" "}
                {unmatchUser?.user && `With ${unmatchUser?.user.name}`}?
              </DialogTitle>
              <Card className="flex flex-1">
                <CardHeader>This action is not reversible!</CardHeader>
                <CardContent>
                  <div className="flex items-center gap-2 justify-between">
                    ...
                    <Button
                      onClick={() => {
                        if (!unmatchUser) {
                          toast.error("Could not unmatch with user!");
                        } else {
                          deleteMatch(unmatchUser);
                        }
                        setUnmatchUser(null);
                        setUnmatchDialog(false);
                      }}
