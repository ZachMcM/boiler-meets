...
export async function testProfileDataIntegrity(
  index: number
): Promise<UnitTestOutputType> {
  const testModules: DraggableModule[] = [
    ...
    },
    {
      ...
      visible: true,
      data: [
        { id: 1, content: "Quality Time", selectedOption: "Quality Time" },
      ],
    },
    {
      id: 3,
    ...
    async () => {
      try {
        ...
        const dimensionsMatch =
          data[0].gridWidth === 2 && data[0].gridHeight === 1;
        return {
...
export async function testMessagingAndAPI(
  index: number
): Promise<UnitTestOutputType> {
  const tests = [
    async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_SERVER_URL}/matches`,
          {
            method: "GET",
            credentials: "include",
          }
    ...
    async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_SERVER_URL}/user/username/test-username`,
          {
            method: "GET",
            credentials: "include",
          }
...
export async function testEndCallFlow(
  index: number
): Promise<UnitTestOutputType> {
  const tests = [
    async () => {
  ...
  const tests = [
    ...
    async () => {
      try {
        ...
        const stateCorrectAfterClick =
          callAgainButtonClicked && waitingUserResponse;
        ...
        const stateCorrectAfterUnclick =
          !callAgainButtonClicked && !waitingUserResponse;

        return {
          success: stateCorrectAfterClick && stateCorrectAfterUnclick,
          content:
            stateCorrectAfterClick && stateCorrectAfterUnclick
    ...
    async () => {
      try {
        ...
        const stateCorrectAfterUnmatch =
          !userHasMatched && !waitingUserResponse;

        return {
          success: stateCorrectAfterClick && stateCorrectAfterUnmatch,
          content:
            stateCorrectAfterClick && stateCorrectAfterUnmatch
...
export async function testMatchSearch(
  index: number
): Promise<UnitTestOutputType> {
  const tests = [
    async () => {
      try {
        ...
        return {
          ...
          content:
            response.ok && Array.isArray(matches)
    ...
    async () => {
      try {
        ...
        const correctResult =
          filtered.length === 1 && filtered[0].user.name === "Alice Johnson";
...
export async function testProfileOnCall(
  index: number
): Promise<UnitTestOutputType> {
  const tests = [
    async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_SERVER_URL}/users/test-user-id`,
          {
            method: "GET",
            credentials: "include",
          }
